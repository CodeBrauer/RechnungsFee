...
`SCHEMA_VERSION = 160` – zentrale Konstante (wird in `main.py` gepflegt).
...
| 139 | rechnungen.eingabemodus VARCHAR(10) DEFAULT 'netto' – Issue #332: eine einzige Berechnung pro Dokument statt mehrerer unabhängiger Rundungswege (Formular-Vorschau/Speichern/PDF); gerundet wird auf POSITIONSEBENE (Einzelpreis × Menge, nicht pro Stück – sonst vervielfacht sich der Rundungsfehler mit der Menge, z.B. 30×2,94€ ergab vorher 21,60€ USt statt korrekt 21,55€); Netto-Richtung summiert Netto pro Position und leitet USt daraus ab, Brutto-Richtung summiert Brutto und leitet Netto daraus ab – Netto+USt=Brutto ist dadurch in beiden Richtungen exakt; USt/Netto wird dabei aus der noch ungerundeten Positionssumme abgeleitet statt aus dem bereits gerundeten Anker (Doppelrundung vermieden – sonst konnte eine Netto-Rechnung z.B. bei aus Brutto-Artikelpreisen abgeleiteten Einzelpreisen einen Cent von der rechnerisch gleichwertigen Brutto-Rechnung abweichen); `rechnungspositionen.ust_betrag`/`.brutto` speichern seitdem die Positionssumme (nicht mehr den Stückpreis) – betrifft auch §25a-Marge, Gutschrift-Vorzeichen, Lieferschein-Konvertierung, Artikel-Verkaufshistorie; neuer Endpunkt POST /rechnungen/vorschau nutzt dieselbe Funktion für die Live-Vorschau in Rechnung/Angebot/Auftrag/Proforma |
| 140 | artikel.vk_netto NUMERIC(12,2)→NUMERIC(12,4) – Issue #332 Folgefehler: vk_netto wurde aus vk_brutto abgeleitet und auf den Cent gerundet gespeichert (3,50€ brutto → 2,94€ statt exakt 2,9412€); bei Rechnungen mit diesem Artikel führte das bei größeren Mengen zu spürbaren Abweichungen zwischen Netto- und Brutto-Rechnung (5 statt 1 Cent). Bestehende Artikel werden einmalig aus vk_brutto neu abgeleitet (außer bei Differenzbesteuerung, dort bleibt vk_netto=vk_brutto); Frontend (`fillPositionFromArtikel` in Rechnung/Angebot/Auftrag/Proforma/Wiederkehrend) übernimmt vk_netto jetzt ungerundet in die Position statt `.toFixed(2)` |
