...
export const CHANGELOG: ChangelogVersion[] = [
  {
    ...
    eintraege: [
      ...
      { typ: 'fix', text: 'Der allererste Druckvorgang einer Rechnung bekam beim Speichern über den PDF-Betrachter im Druckfenster fälschlich schon den KOPIE-Stempel, statt als echtes Original zu gelten – der Speichern-Button des Betrachters fragt die Rechnung technisch bedingt ein zweites Mal ab, nachdem das Original durch die erste Anzeige bereits archiviert wurde. Danke an duczz für den Screen-Recording-Report (Issue #394).' },
      { typ: 'fix', text: 'Eingangsrechnungen und Ausgaben im Kassenbuch zeigten bei aktivem §19 UStG (Kleinunternehmer) fälschlich immer 0 % USt, das Feld war komplett gesperrt – dabei betrifft die Kleinunternehmerregelung nur die eigenen Ausgangsumsätze, nicht die von Lieferanten in Rechnung gestellte USt. Eine Eingangsrechnung über 100 € netto + 19 % USt wurde dadurch zu 100 € statt korrekt 119 € Brutto. Der USt-Satz ist bei Eingangsrechnungen und Ausgaben jetzt frei wählbar wie gewohnt, nur der Vorsteuerabzug bleibt für Kleinunternehmer weiterhin gesperrt. Danke an Eberts für den Bug-Report (Issue #397).' },
      { typ: 'fix', text: 'Bank-Import DKB: Eine Kontoauszugs-CSV mit einem unsichtbaren BOM-Zeichen am Dateianfang (kommt bei modernen Exporten häufig vor) ergab 0 erkannte Umsätze, weil dieses Zeichen sich vor den ersten Spaltennamen schob und der Abgleich dadurch für jede Zeile scheiterte. Zusätzlich wurden die Kopfzeilen vor der eigentlichen Spaltenüberschrift eines echten DKB-Exports (Kontobezeichnung, Kontostand) nicht automatisch übersprungen. Beides betraf nur DKB-Dateien im Originalzustand – wer die Kopfzeilen manuell entfernt hatte, war vom zweiten Punkt nicht betroffen. Danke an derdobbi für den präzisen Bug-Report (Issue #398).' },
      { typ: 'fix', text: 'Wiederkehrende Rechnungen berücksichtigten den Kleinunternehmerstatus (§19 UStG) nicht: Der Eingabemodus wurde rein anhand Gewerbe-/Privatkunde gewählt, der USt-Satz war frei wählbar statt auf 0 % gesperrt. Bei einer Brutto-Eingabe (z. B. an Privatkunden) wurde der eingegebene Betrag dadurch intern mit einem fiktiven 19-%-Satz heruntergerechnet und genau dieser bereits falsche Betrag in der Vorlage gespeichert – aus 119,00 € eingegeben wurden dauerhaft 100,00 € Netto/Brutto. Der USt-Satz ist bei wiederkehrenden Rechnungen jetzt wie bei normalen Ausgangsrechnungen fest auf 0 % gesperrt, sobald das Unternehmen als Kleinunternehmer hinterlegt ist. Wichtig: Bereits bestehende wiederkehrende Rechnungen, die während des Bugs mit einem zu niedrigen Betrag angelegt wurden, bleiben unverändert gespeichert und müssen einmalig manuell auf den korrekten Betrag korrigiert werden – der Fix verhindert nur, dass der Fehler bei neuen oder bearbeiteten Vorlagen erneut auftritt. Danke an derdobbi für den Bug-Report (Issue #400).' },
      { typ: 'fix', text: 'Nebenfund beim Beheben von Issue #400: Buchungsvorlagen ("Wiederkehrende Buchung" für Miete/Leasing/Abonnements im Kassenbuch) kannten den Kleinunternehmerstatus ebenfalls nicht und ließen den USt-Satz für Einnahmen wie Ausgaben frei wählen. Bei einer Ausgabe (z. B. Miete) hat das Backend daraus automatisch einen echten Vorsteuerabzug berechnet, ohne den Kleinunternehmerstatus zu prüfen – ein Kleinunternehmer hätte darüber bei jeder Ausführung unzulässig Vorsteuer gutgeschrieben bekommen, die direkt in die EÜR (Zeile 57) eingeflossen wäre. Der reale USt-Satz von Vermieter/Lieferant bleibt jetzt erhalten (wie bei Eingangsrechnungen, Issue #397), nur der Vorsteuerabzug entfällt für Kleinunternehmer zuverlässig serverseitig. Bei Einnahme-Vorlagen ist der USt-Satz für Kleinunternehmer jetzt ebenfalls auf 0 % gesperrt. Anders als bei den wiederkehrenden Rechnungen oben: Bereits bestehende, über eine Buchungsvorlage erzeugte Buchungen werden beim ersten Start dieser Version automatisch korrigiert (Vorsteuerabzug zurückgenommen bzw. USt-Ausweis entfernt), eine manuelle Korrektur ist hier nicht nötig.' },
      { typ: 'fix', text: 'Vor dem eigentlich frei konfigurierbaren Nummernkreis-Format für Ausgangs-/Eingangsrechnungen stand bisher zusätzlich fest im Code ein "RE-"/"ER-"-Präfix – eine eigene Rechnungsnummern-Konvention wie „R001-2026" wurde dadurch immer zu „RE-R001-2026" verfälscht. Das Format entscheidet jetzt allein über die erzeugte Nummer; wer „RE-" möchte, trägt es direkt im Format ein (Standard bleibt „RE-YY####"/„ER-YY####", damit sich für alle, die nichts ändern, am Ergebnis nichts ändert). Danke an derdobbi für den Bug-Report (Issue #399).' },
      { typ: 'neu', text: 'Eigener, optionaler Nummernkreis für wiederkehrende Rechnungen: Auf der Nummernkreise-Seite lässt sich ein separater Zähler für aus Vorlagen erzeugte Ausgangsrechnungen aktivieren, damit sie nicht mehr zwangsläufig die laufende Nummernfolge der normalen Rechnungen mit hochzählen. Standardmäßig aus (gemeinsamer Kreis wie bisher); einmal aktiviert und genutzt lässt er sich nicht mehr zurückschalten, um doppelt vergebene Rechnungsnummern auszuschließen (Issue #399, Wunsch 2).' },
    ],
  },
  {
    version: 'v0.6.12',
    ...
    eintraege: [
      ...
      { typ: 'fix', text: 'Automatische Erlöskonto-/Kategoriewahl bei Rechnungen mit gemischten USt-Sätzen (19 % und 7 % in derselben Rechnung) konnte bei unterschiedlichen Mengen den falschen Satz als „dominant" auswählen, da die Ermittlung auf Stückpreisen statt auf den tatsächlichen Positionssummen beruhte – betraf sowohl die Zahlungsbuchung als auch den DATEV-Export-Fallback (Nebenfund zu Issue #389).' },
      { typ: 'fix', text: 'UStVA-Anzeigehilfe: §13b-/innergemeinschaftliche Alt-Buchungen aus der Zeit vor der Sonderfall-Erkennung (Migration 375) erschienen weiterhin nicht in KZ 46/47, 84/85 oder 67, obwohl ihre Kategorie längst korrekt zugeordnet war – ein früherer Datenfix hatte nur bereits falsch getaggte, nicht aber komplett ungetaggte Alt-Buchungen erfasst. Danke an UweKoslowski für die sehr genaue Ursachenanalyse (Issue #372).' },
      { typ: 'verbesserung', text: 'Das beim ersten Drucken/Mailen archivierte Original-PDF heißt jetzt nach der Rechnungsnummer statt nach der internen Datenbank-ID (z. B. „RE-260103_42.pdf" statt „42.pdf") – dadurch auf Anhieb erkennbar und zwischen Eingang (ER-) und Ausgang (RE-) unterscheidbar. Frei wählbarer Speicherort bleibt auf der Roadmap. Vorschlag von Micha (Issue #390).' },
    ],
  },
  {
    version: 'v0.6.11',
  ...
  },
  {
    ...
    eintraege: [
      ...
      { typ: 'neu', text: 'Profile umbenennen, archivieren und wiederherstellen: Auf der Seite „Profile" lässt sich jedes Profil jetzt umbenennen (✏️) und archivieren (📥) – Archivieren blendet es nur aus der Liste aus, Datenbank, Belege und Backups bleiben dabei unangetastet und lassen sich jederzeit über „Archivierte Profile" wiederherstellen. Erst von dort aus ist ein Profil zusätzlich endgültig löschbar, mit Sicherheitsabfrage per Eingabe des Profilnamens (das aktive Profil lässt sich weder archivieren noch löschen).' },
      { typ: 'neu', text: 'Einleitungs- und Schlusstext getrennt je Dokumenttyp: Bisher galt ein einziger Standardtext für alle Dokumente – ein für Rechnungen passender Satz ("...stellen wir folgende Leistungen in Rechnung") erschien dadurch fälschlich auch auf Angeboten, Aufträgen, Proformarechnungen und Lieferscheinen. Unter Einstellungen → Unternehmen → Funktionen lässt sich jetzt für jeden aktivierten Dokumenttyp ein eigener Standard-Einleitungs- und -Schlusstext hinterlegen; zusätzlich weiterhin frei überschreibbar pro einzelnem Dokument in Angebot, Auftrag, Proforma und Rechnung.' },
      { typ: 'fix', text: 'Ein bereits als Entwurf gespeicherter Einleitungstext auf einer Rechnung ging beim erneuten Bearbeiten verloren, da das Update-Formular das Feld nicht kannte – nur beim Neuanlegen wurde es korrekt übernommen.' },
      { typ: 'fix', text: 'Ein aus einem Angebot erstellter Auftrag übernahm dessen Einleitungstext nicht.' },
      { typ: 'fix', text: 'Angebot, Auftrag und Proformarechnung erkannten den Kleinunternehmer-Status nicht: Neue Positionen bekamen immer 19 % USt als Vorschlag, und der USt-Satz ließ sich pro Position frei umstellen, obwohl nach § 19 UStG gar keine Umsatzsteuer berechnet werden darf. Das USt-Feld ist dort jetzt wie bei Rechnungen fest auf 0 % gesperrt.' },
      { typ: 'fix', text: 'Die Option „Profile aktivieren" unter Einstellungen → Unternehmen → Funktionen ließ sich anscheinend ausschalten, obwohl der Menüpunkt „Profile" ab dem zweiten Profil ohnehin dauerhaft sichtbar bleibt. Der Haken ist jetzt in diesem Fall fest gesetzt und ausgegraut, damit kein falscher Eindruck entsteht.' },
      { typ: 'fix', text: 'Profilnamen mit Umlauten (z. B. „Müller GmbH", „Bäckerei") wurden als ungültig abgelehnt, obwohl die Fehlermeldung „nur Buchstaben, Zahlen, …" etwas anderes vermuten ließ. Umlaute und ß sind jetzt zulässig.' },
    ],
  },
  {
    version: 'v0.6.0',
  ...
  },
  {
    ...
    datum: 'August 2026',
    eintraege: [
      { typ: 'fix', text: 'UStVA – das Stornieren einer Buchung mit steuerlichem Sonderfall (ig. Erwerb, §13b Abs. 1/2) verdoppelte statt sie aufzuheben: Bemessungsgrundlage und Steuer (z. B. KZ 46/47) addierten sich, nur die Vorsteuer (KZ 67/61) hob sich korrekt auf – die Zahllast stieg dadurch nach einem Storno an, statt auf 0,00 € zurückzugehen. Ursache: Der Storno-Endpunkt legt sein Vorzeichen nur in der Buchungsart ab (Einnahme/Ausgabe invertiert), die Sonderfall-Auswertung in der UStVA prüfte diese aber nicht und behandelte Original- und Stornobetrag gleich. Sichtbar wurde der seit Langem bestehende Fehler erst mit v0.5.4, weil die Vorsteuer bei diesen Kategorien vorher meist 0 war (Issue #339) und dadurch nicht auffiel, dass auch die Bemessungsgrundlage sich nicht aufhob. Danke an flole für den sehr präzisen Report inkl. vollständig durchgetesteter Szenarien (Issue #344).' },
      { typ: 'fix', text: 'Journal – sechs mitgelieferte Privat-Kategorien für „Absetzungen vom Einkommen" (Einkommensteuer-Vorauszahlung, Kranken-/Pflege-/Rentenversicherung, Riester-Beiträge, Sonstige Absetzungen) hatten keine EÜR-Zeile hinterlegt und erschienen dadurch seit der letzten Version im Buchungsformular sowohl unter „Einnahme" als auch „Ausgabe", obwohl sie wirtschaftlich immer Privatentnahmen sind. Jetzt korrekt auf Zeile 106 (Privatentnahme) eingeordnet und nur noch unter „Ausgabe" sichtbar. Danke an flole fürs Weitertesten (Issue #341-Folgefund).' },
      { typ: 'fix', text: 'Skonto war beim Bezahlen einer Eingangsrechnung nicht auswählbar – weder das Skonto-%/Frist-Feld im Rechnungsformular noch die Skonto-Option im Zahlungsdialog waren dafür vorgesehen, obwohl das Backend mit einem eigenen „Erhaltene Skonti"-Konto längst dafür ausgelegt ist. Wer den Zahlbetrag deshalb von Hand um den Skonto-Abzug reduziert hat, blieb mit einem dauerhaft offenen Restbetrag zurück, weil kein Skonto-Buchungseintrag den fehlenden Betrag ausgeglichen hat. Danke an Peter1061 fürs Nachfragen (Issue #343-Folgefund).' },
      { typ: 'fix', text: 'Vorsteuerabzug nach Soll-Prinzip (§15 UStG, v0.5.4) wurde nur erfasst, wenn eine Eingangsrechnung über den eigenständigen „Finalisieren"-Button bei einem bereits gespeicherten Entwurf abgeschlossen wurde. Die beiden anderen, mindestens genauso gängigen Wege – eine Rechnung direkt beim Anlegen fertigstellen, oder einen Entwurf über „Speichern & Finalisieren" abschließen – lösten die Vorsteuer-Erfassung gar nicht erst aus, ohne dass irgendein Fehler sichtbar wurde. Betraf nur Eingangsrechnungen mit Datum ab dem 01.10.2026. Danke an Peter1061 für das sehr gründliche gemeinsame Eingrenzen über mehrere Schritte (Issue #343-Folgefund).' },
      { typ: 'fix', text: 'PDF-/Datei-Downloads (z. B. „Mail senden" ohne SMTP, DATEV-/GoBD-/Buchhalter-Export, Inkasso-Paket, Mahnungs-PDF) waren unter Linux nicht zuverlässig – je nach Distribution löste der Klick gar keinen sichtbaren Download aus (beobachtet z. B. unter CachyOS), während er auf anderen Systemen funktionierte (z. B. Linux Mint). Ursache: Der bisherige Browser-Download-Mechanismus hängt von der jeweiligen WebKitGTK-Version/Desktop-Umgebung ab und verhält sich dadurch uneinheitlich. Alle Downloads laufen jetzt distributionsunabhängig über denselben nativen Speichern-Dialog, wie er bereits beim Backup-Download zuverlässig funktionierte.' },
      { typ: 'verbesserung', text: 'Feld „Fällig am" heißt jetzt „Fällig bis" (Rechnungsformular, Rechnungsliste, Detail-Panel, Dashboard, Mahnungs-/Inkasso-PDF, CSV-Export, Standard-Mailtext) – eindeutiger, da es sich um eine Zahlungsfrist handelt, nicht um ein festes Zahlungsdatum. Danke an gitmacer für den Vorschlag (Issue #345).' },
    ],
  },
  {
    version: 'v0.5.4',
  ...
  },
  {
    ...
    eintraege: [
      ...
      { typ: 'fix', text: 'Buchungsvorlagen – eine ausgelöste, überfällige Vorlage (z. B. seit Monaten fällige Miete) wurde mit dem heutigen Datum statt dem tatsächlichen Fälligkeitsdatum gebucht (Issue #314).' },
    ],
  },
  {
    version: 'v0.4.25',
  ...
  },
  {
    ...
    eintraege: [
      ...
      { typ: 'fix', text: 'Reverse Charge – die automatische Erkennung von §13b/innergemeinschaftlichem Erwerb hing an drei Kontonummern (SKR03 3400/3300/3610), die tatsächlich normale Wareneingangs- bzw. Vorsteuerkonten ohne jeden Bezug zu Reverse Charge sind. Eigene Kategorien auf genau diesen Konten (z. B. normaler Inlands-Wareneinkauf) wurden dadurch fälschlich als Reverse Charge gebucht. Korrigiert auf die tatsächlichen DATEV-Automatikkonten (3425/3123/3120, SKR04 5425/5923/5920), gegengeprüft am offiziellen DATEV-Kontenrahmen. Danke an flole (Issue #308).' },
      { typ: 'fix', text: 'Journal – beim Bearbeiten einer Buchung mit steuerlichem Sonderfall (§13b/i.g. Erwerb) zeigte das Formular je nach Ladereihenfolge der Hintergrunddaten „— keiner —" statt des gespeicherten Werts an und löschte ihn beim Speichern still. Betraf alle §13b-Kategorien außer den drei Standard-Reverse-Charge-Kategorien. Danke an flole (Issue #309).' },
    ],
  },
  {
    version: 'v0.4.23',
